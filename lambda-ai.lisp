...
(defun wave3 (map front fruit-ticks ghost-hunter power-pills-num)
  ...
  (let ((orig-map map)
        ...
        (power-pills-hunter (> power-pills-num 0)))
...
(defun mark-ghost-ways (map lm-coord ghosts ghost-hunter)
  ...
  (labels ((%ghost (map g)
             (mark-way map lm-coord
                       (ghost-coord g) (ghost-direction g)))
           (%mark (map ghosts)
             (if (null ghosts)
                 map
                 (let ((g (car ghosts)))
                   ...
                   (%mark
                    (mcase (ghost-vitality g)
                           (+vitality-standard+ (%ghost map g))
                           (+vitality-invisible+ (if ghost-hunter
                                                     (%ghost map g)
                                                     map))
...
(defun make-wave-step ()
  (lambda (power-pills-num world-state)
    (let* ((lambda-man (world-state-lambda-man world-state))
           (ghost-hunter (> (lambda-man-vitality lambda-man)
                            (* 137 +frighten-mode-reserve-ticks+)))
           (lambda-man-coords (lambda-man-coord lambda-man))
           (map (get-trie-for-map (car world-state)))
           (map (mark-ghost-ways map lambda-man-coords
                                 (world-state-ghosts world-state)
                                 ghost-hunter)))
